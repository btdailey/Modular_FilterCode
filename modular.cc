void MyCorrelator::getNotchandBandwidth(int nfreq,double dBCut,int nAntennasToUse,int nadirFlag,vector< vector<double> > &notchFreqs,vector< vector<double> > &notchBands ,vector< vector<double> > &notchFreqs_Horiz,vector< vector<double> > &notchBands_Horiz ,vector< vector<double> > &notchPhase,vector< vector<double> > &notchPhaseBands ,vector< vector<double> > &notchPhase_Horiz,vector< vector<double> > &notchPhaseBands_Horiz,float &meanFreqVert, float &meanFreqHoriz){
  //gets Notch locations and Bandwidth of each notch for every unique group of antennas
  ...
  for(int i =0;i<40;i++){//WE have all possible freqs to cut. Let go through each antenna and make sense of them
...
/////////////////////////
void MyCorrelator::getGroupsofAntennas(int nAntennasToUse, int nadirFlag){//gets unique groups of Antennas
...
///////////////////////////////////////
void MyCorrelator::adaptiveFilterPartialPayload(int pol, double dBCut, int nfreq,double *frequencies,double *bandwidth,double *magPeak,int nantennasToUse,vector<int>& whichAntennasToUse, float &mean_freq)//identifies CW frequencies and size of notch for group of antennas
{
  ...
  for (int ctr=0;ctr<nantennasToUse;ctr++){
    ...
    if ((pol!=0 || ant!=1) && saturatedChannels[ant+pol*NUM_ANTS_WITH_NADIRS]==0){//get rid of 2V and ant 1 in vert
...
void MyCorrelator::readBaselineFFTs_Brian() //read in individual Baseline and keep track of them
{
...
//////////////////////////////////
void MyCorrelator::GetBaselineperPhi(int pol, double *baseline, int nantennasToUse, vector<int>& whichAntennasToUse ){//get baseline for groups of antennas
...
///////////////////////////////
void MyCorrelator::applyAdaptiveFilter_singleAnt(double centerFrequency, double bandWidth, int polFlag,int ant) //function to call coorrect filter and change graphs
{
...
TGraph *MyCorrelator::interpolatedFilter(TGraph *grWave, Double_t minFreq, Double_t maxFreq) //interpolated filter. Do interpolation across notch
{
  if(maxFreq>1200){
    maxFreq=1200;
  }
...
//////////////////////////////////////
void MyCorrelator::GeomMethod(int ant,int pol,vector<double> Freq,vector<double> bandWidth,vector<double> cutFreqs){ //geom method for phase filtering
